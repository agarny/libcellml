    ...
    if (!mProfile->objectiveFunctionMethodString(modelHasOdes(mAnalyserModel), mAnalyserModel->hasExternalVariables()).empty()
        ...
        for (const auto &analyserEquation : mAnalyserModel->analyserEquations()) {
            if ((analyserEquation->type() == AnalyserEquation::Type::NLA)
                ...
                for (const auto &analyserVariable : analyserVariables) {
                    // TODO: ---GRY--- Re-enable once all of our tests are back to normal...?
                    /*
...
// TODO: ---GRY--- Check but I believe this method should not be needed anymore since it was used to initialise a variable (to zero) that is not on its own on either the LHS or RHS of an equation and that therefore needed to be computed using an NLA equation. However, this is now handled by SymEngine.
/*
                                                                           ...
                                                                           std::vector<AnalyserVariablePtr> &remainingAlgebraicVariables)
{
    ...
    // TODO: ---GRY--- Check but I believe this should not be needed anymore (thanks to SymEngine).
    /*
    for (const auto &state : mAnalyserModel->states()) {
