                                                ...
                                                const AnalyserEquationAstPtr &astChild)
{
    ...
    if ((astChild->type() == AnalyserEquationAst::Type::CI)
        && (variable->mVariable == astChild->variable())) {
...
void Analyser::AnalyserImpl::analyseModel(const ModelPtr &model)
{
    ...
    for (const auto &internalEquation : mInternalEquations) {
        ...
        case AnalyserInternalEquation::Type::NLA: {
            ...
            for (const auto &nlaSibling : internalEquation->mNlaSiblings) {
                if (auto lockedNlaSibling = nlaSibling.lock()) {
                    addNlaSystemUnknowns(lockedNlaSibling);
                }
            }
    ...
    // If an equation still contains some unknown variables then they should be marked as underconstrained.
    ...
    for (const auto &internalEquation : mInternalEquations) {
        ...
        if (equationType == AnalyserEquation::Type::EXTERNAL) {
            ...
            const auto &unknownVariables = internalEquation->mUnknownVariables;
            ...
            for (const auto &equationVariable : internalEquation->mAllVariables) {
                if (std::find(unknownVariables.begin(), unknownVariables.end(), equationVariable) != unknownVariables.end()) {
                    continue;
                }
