...
void Analyser::AnalyserImpl::analyseModel(const ModelPtr &model)
{
    ...
    // not initialised using a linear algebraic variable.

    for (const auto &internalVariable : mInternalVariables) {
        if ((internalVariable->mInitialisingVariable != nullptr)
            && !internalVariable->mInitialisingVariable->initialValue().empty()
            ...
            if (initialisingInternalVariable->mType == AnalyserInternalVariable::Type::ALGEBRAIC) {
                ...
                issue->mPimpl->setDescription("Variable '" + internalVariable->mVariable->name()
                                              ...
                                              + "', which is a linear algebraic variable. Only a reference to a constant, a computed constant, a state variable, or a non-linear algebraic variable is allowed.");
